...
export default function DashboardPage() {
    ...
    const loadDashboardData = async () => {
        try {
            ...
            const ordersResponse = await fetch('http://localhost:3001/api/active-orders');
                ...
                <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 hover:border-slate-600 transition-colors">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-white mt-1">{stats.activeOrders}</p>
                        </div>
                ...
                <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 hover:border-slate-600 transition-colors">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-white mt-1">{stats.criticalStock}</p>
                        </div>
                ...
                <div className="bg-slate-800 p-4 rounded-lg border border-slate-700 hover:border-slate-600 transition-colors">
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-white mt-1">{stats.activeRecipes}</p>
                        </div>
                            ...
                            <table className="w-full">
                                ...
                                <tbody>
                                    {activeOrders.length === 0 ? (
                                        <tr className="border-b border-slate-700">
                                            <td className="p-2 text-slate-300" colSpan={4}>Henüz aktif sipariş yok</td>
                                        </tr>
                                    ) : (
                                        activeOrders.map(order => (
                                            <tr key={order.id} className="border-b border-slate-700">
                                                <td className="p-2 text-slate-300">{order.customer_name}</td>
                                                <td className="p-2 text-slate-300">{order.recipe_name}</td>
                                                <td className="p-2 text-slate-300">{order.quantity}</td>
                                                <td className="p-2 text-slate-300">{order.total.toLocaleString('tr-TR')} TL</td>
                                            </tr>
                                        ))
                                    )}
                                </tbody>
