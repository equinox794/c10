...
export default function SettingsPage() {
  ...
  const loadTeklifSettings = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/teklif-settings`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingPackage
        ? `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/packages/${editingPackage.id}`
        : `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/packages`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/packages/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleBackup = async (type) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/backup/${type}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleRestore = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/backup/restore-file`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const handleUpdatePrice = async (id, newPrice, type) => {
    try {
      const url =
        type === "stock"
          ? `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/stock/${id}/price`
          : `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/packages/${id}/price`;
      ...
      toast.success(
        `Fiyat güncellendi. ${result.updatedCount} reçete güncellendi.`
      );
  ...
  const loadPriceHistory = async (id, type) => {
    try {
      const url =
        type === "stock"
          ? `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/stock/${id}/price-history`
          : `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/packages/${id}/price-history`;
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      toast.success(
        `${result.message} ve ${updateResult.updatedCount} reçete güncellendi.`
      );
  ...
  const handleTeklifSettingsUpdate = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/teklif-settings/${teklifSettings.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Sistem ayarları ve ambalaj yönetimi
          </p>
        </div>
      </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    dolar_kuru: parseFloat(e.target.value),
                  })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    liste_a_kar_orani: parseFloat(e.target.value),
                  })
                }
                className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    liste_b_kar_orani: parseFloat(e.target.value),
                  })
                }
                className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({
                    ...settings,
                    liste_c_kar_orani: parseFloat(e.target.value),
                  })
                }
                className="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white"
              />
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-slate-700 p-6 rounded-lg">
              ...
              <h3 className="text-center text-lg font-medium text-white mb-4">
                Veritabanı Yedekleme
              </h3>
              <button
                  ...
                  <div className="text-green-400">
                    ...
                    {backupStatus.path && (
                      <p className="text-sm mt-1">
                        Dosya: {backupStatus.path} (
                        {formatBytes(backupStatus.size)})
                      </p>
                    )}
                  </div>
                )}
