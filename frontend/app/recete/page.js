...
export default function RecipePage() {
  ...
  const totals = newRecipe.ingredients.reduce(
    (acc, curr) => ({
  ...
  const handleSaveRecipe = async () => {
    ...
    try {
      const url = isEditing
        ? `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/recipes/${editingRecipeId}`
        : `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/recipes`;
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error ||
            (isEditing ? "Reçete güncellenemedi" : "Reçete kaydedilemedi")
        );
      }

      toast.success(
        isEditing
          ? "Reçete başarıyla güncellendi"
  ...
  const handleDeleteRecipe = async (id) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/recipes/${id}`,
        {
          method: "DELETE",
          headers: {
  ...
  const handleUpdatePrices = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/recipes/update-prices`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          result.error || "Fiyatlar güncellenirken bir hata oluştu"
        );
      }
          ...
          <div className="mb-6 bg-slate-800 rounded-lg p-6 shadow-lg">
            <div className="flex justify-between items-center mb-4">
              <div>
                <h3 className="text-xl font-semibold text-white">
                  Reçete Detayları - {selectedRecipe.name}
                </h3>
                <p className="text-sm text-slate-400 mt-1">
                  Şarj Sayısı: {chargeCount}
                </p>
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tfoot>
                  <tr>
                    <td colSpan="2" className="py-4 text-right text-slate-400">
                      Toplam:
                    </td>
                    <td className="py-4 font-bold text-white">
                      {selectedRecipe.ingredients
                        ?.reduce(
                          (total, ing) =>
                            total + parseFloat(calculateAmount(ing.quantity)),
                          0
                        )
                        .toFixed(2)}{" "}
                      kg
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setNewRecipe((prev) => ({ ...prev, customer_id: value }))
                  }
                >
            ...
            <div className="flex justify-between items-center pt-4 border-t border-slate-700">
              <div>
                <p className="text-slate-400">
                  Toplam Miktar: {totals.quantity.toFixed(2)} kg
                </p>
                <p className="text-slate-400">
                  Toplam Maliyet: {totals.cost.toFixed(2)} TL
                </p>
              </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {expandRecipesByPackage(filteredRecipes).map((recipe) => {
                  ...
                  const costPerKg = recipe.total_cost
                    ? recipe.total_cost / 1000
                    : 0;
                  let packagePrice = 0;
                  ...
                  const packageId = recipe.currentPackage
                    ? packages.find(
                        (p) => `${p.size} ${p.unit}` === recipe.currentPackage
                      )?.id
                    : recipe.packages
                    ? recipe.packages[0]
                    : null;
                  ...
                  if (
                    ...
                    packagePrice =
                      costPerKg * packageSize * density + ambalajMaliyeti;
                  }
