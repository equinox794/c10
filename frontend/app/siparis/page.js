...
export default function OrderPage() {
  ...
  const loadPackages = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/packages`
      );
  ...
  const loadRecipes = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/recipes`
      );
  ...
  const loadOrders = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/orders`
      );
  ...
  const loadSettings = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/settings`
      );
  ...
  const filteredRecipes = recipes.filter((recipe) => {
    ...
    const matchesCustomer =
      ...
      recipe.customer_id?.toString() === selectedCustomerFilter;
  ...
  const handleUpdateStatus = async (orderId, newStatus) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/orders/${orderId}/status`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteOrder = async (orderId) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/orders/${orderId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleCalculateChargeCount = async (recipe) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/recipes/${recipe.id}`
      );
  ...
  const calculateUnitPrice = (recipe, packageInfo) => {
    ...
    try {
      ...
      const currentPackage = packages.find(
        (p) => p.id === parseInt(currentPackageId)
      );
      ...
      if (packageUnit.toUpperCase() === "L") {
        ...
        const density = parseFloat(recipe.density) || 1.2;
      ...
      const aListeFiyat =
        ambalajMaliyet * (1 + settings.liste_a_kar_orani / 100); // En ucuz
  ...
  const handleAddOrder = async (recipe) => {
    ...
    try {
      ...
      const packageInfo = recipe.currentPackage
        ? recipe.currentPackage.split(" ")
        : ["0", "L"];
  ...
  const handleTeklifOlustur = async () => {
    ...
    const secilenUrunler = Object.entries(recipeAdet)
      ...
      .map(([displayId, adet]) => {
        const recipe = expandRecipesByPackage(filteredRecipes).find(
          (r) => r.displayId === displayId
        );
        ...
        return {
          ...
          birimFiyat:
            paraBirimi === "TL" ? birimFiyat : calculateDolarPrice(birimFiyat),
          toplamFiyat:
            paraBirimi === "TL"
              ? birimFiyat * adet
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/teklif/create-pdf`,
        {
          method: "POST",
          headers: {
      ...
      link.href = url;
      link.download = `Teklif_${teklifMusterisi}_${new Date()
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredOrders.map((order) => {
                        ...
                        const packageInfo = recipe?.packages?.[0]
                          ? packages.find(
                              (p) => p.id === parseInt(recipe.packages[0])
                            )
                          : null;
                                ...
                                {order.status === "Beklemede" && (
                                  <>
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        handleUpdateStatus(order.id, "İptal")
                                      }
                                    >
                                      <X size={18} />
                                    </Button>
                      ...
                      >
                        ...
                        <SelectContent className="bg-slate-800 border-slate-700">
                          {customers
                            .filter(
                              (c) =>
                                c.type === "Müşteri" || c.type === "Bioplant"
                            )
                            .map((customer) => (
                              <SelectItem
                                key={customer.id}
                  ...
                  <table className="w-full">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                const recipesToCalculate =
                                  expandRecipesByPackage(
                                    filteredRecipes
                                  ).filter(
                                    (recipe) => recipeMiktar[recipe.displayId]
                                  );

                                if (recipesToCalculate.length === 0) {
                                  toast.error(
                                    "Lütfen en az bir reçete için şarj sayısı girin"
                                  );
                                  return;
                                }
                                ...
                                Promise.all(
                                  recipesToCalculate.map(async (recipe) => {
                                    try {
                                      const response = await fetch(
                                        `${process.env.NEXT_PUBLIC_API_ORIGIN}/api/recipes/${recipe.id}`
                                      );
                                      if (!response.ok)
                                        throw new Error(
                                          "Reçete detayları alınamadı"
                                        );
                                      const recipeDetails =
                                        await response.json();

                                      // Şarj sayısını 5.5 ile çarp
                                      const inputChargeCount = parseFloat(
                                        recipeMiktar[recipe.displayId]
                                      );
                                      const chargeCount =
                                        inputChargeCount * 5.5;
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                const recipesToAdd = expandRecipesByPackage(
                                  filteredRecipes
                                ).filter(
                                  (recipe) => recipeAdet[recipe.displayId]
                                );

                                if (recipesToAdd.length === 0) {
                                  toast.error(
                                    "Lütfen en az bir reçete için adet girin"
                                  );
                                  return;
                                }

                                Promise.all(
                                  recipesToAdd.map(async (recipe) => {
                                    try {
                                      const packageInfo = recipe.currentPackage
                                        ? recipe.currentPackage.split(" ")
                                        : ["0", "L"];
                                      const birimFiyat = calculateUnitPrice(
                                        recipe,
                                        packageInfo
                                      );
                                      const adet = parseFloat(
                                        recipeAdet[recipe.displayId]
                                      );
                                  ...
                                  .catch((error) => {
                                    ...
                                    toast.error(
                                      "Siparişler oluşturulurken hata oluştu"
                                    );
                                  });
                              }}
                            >
                    ...
                    <tbody>
                      {expandRecipesByPackage(filteredRecipes).map((recipe) => (
                        <tr
                          key={recipe.displayId || recipe.id}
                        ...
                        >
                          ...
                          <td className="py-4 text-white">
                            {customers.find((c) => c.id === recipe.customer_id)
                              ?.name || "-"}
                          </td>
                          <td className="py-4 text-white">
                            {recipe.currentPackage ||
                              recipe.packages
                                .map((pid) => {
              ...
              <div className="mt-8 bg-slate-800 rounded-lg p-4 shadow-lg">
                ...
                <div className="bg-slate-700/30 rounded-lg p-3">
                  <h3 className="text-base font-medium text-white mb-2">
                    Hammadde İhtiyacı ve Stok Durumu
                  </h3>
                  <div className="overflow-x-auto">
                    <table className="w-full border-collapse">
                      <thead>
                        <tr className="text-left text-slate-400 border-b border-slate-700">
                          ...
                          <th className="pb-2 px-4 text-right w-32">
                            Gereken (kg)
                          </th>
                          <th className="pb-2 pl-4 text-right w-32">
                            Stok Durumu (kg)
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(calculateTotalIngredients())
                          .filter(
                            ([name]) => !name.toLowerCase().includes("su")
                          )
                          .map(([name, total], index) => {
                            const stockNeeded = calculateStockStatus(
                              name,
                              total
                            );
                            return (
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-4 items-center">
                  ...
                  <Select value={paraBirimi} onValueChange={setParaBirimi}>
                    ...
                    <SelectContent className="bg-slate-800 border-slate-700">
                      <SelectItem value="TL" className="text-white">
                        TL
                      </SelectItem>
