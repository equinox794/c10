...
export const api = {
  ...
  addStocks: (stocks: Partial<Stock>[]) => fetchApi<Stock[]>('/stock/bulk', {
    method: 'POST',
    headers: {
  ...
  updateStock: (id: number, stock: Partial<Stock>) => fetchApi<Stock>(`/stock/${id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
    },
  ...
  deleteStock: (id: number) => fetchApi<void>(`/stock/${id}`, {
    method: 'DELETE',
  }),
